...
> ⚠ **图片本体不加任何文字 / Emoji / 水印**——本系统的所有 JSON 模板都已移除"装饰叠层"字段。Emoji 列仅作为小红书**标题/正文**的配套建议，不进入图片提示词。

| 风格主线 | 灯光 ｜ 主光 | 灯光 ｜ 色温 | 背景 ｜ 色彩 | 风格 ｜ 氛围词 | 标题用 Emoji（不上图）|
|---|---|---|---|---|---|
...
> ⚠ **前 5 条专治"AI 自动在图上加字"**——GPT-image-2 / Nano Banana 2 默认会倾向于在图上加品牌名 / 字幕 / Emoji 等装饰元素，前 5 条 negative 是关键约束。**所有 3 张图的 negative 都必须包含这 5 条**。
...
A：可以删除以下字段不影响主结构：
- `画质标签` ← 可省
- `禁止出现` 后半段（保留前 5 条防图上加字的约束）

保留 5 个核心字段：`人物锁` `妆容` `灯光` `背景` `画面比例`。
**禁止出现的前 5 条务必保留**——专治 AI 默认在图上加字幕/Emoji/品牌名。
